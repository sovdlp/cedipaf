...
(function (global, factory) {
  ...
  function useLocation(props) {
    ...
    const locationStyles = vue.computed(() => {
      ...
      if (side !== 'center') {
        if (opposite) styles[oppositeMap[side]] = `calc(100% - ${vue.unref(offset)}px)`;else styles[side] = 0;
      }

      if (align !== 'center') {
        if (opposite) styles[oppositeMap[align]] = `calc(100% - ${vue.unref(offset)}px)`;else styles[align] = 0;
      } else {
  ...
  const VBadge = defineComponent({
    ...
    setup(props, ctx) {
      ...
      const {
        locationStyles
      } = useLocation(props, true, vue.computed(() => {
  ...
  const version = "3.0.0-next-20220529.0";
